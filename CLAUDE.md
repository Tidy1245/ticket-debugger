...
| POST | `/api/tickets/{id}/vlm-check-answer` | VLM 檢查答案（SSE 串流，比對 table data 正確性） |
...
### VLM Check Answer 功能
- 獨立 Tab「VLM Check」，用 VLM 檢查 OCR pipeline 的 table data 是否正確
- 後端 SSE 串流端點 `POST /api/tickets/{id}/vlm-check-answer`
- 流程：OCR keywords 篩選 table 頁面 → 每 2 頁拼接（PIL）→ 送 VLM 比對 → SSE 回傳進度
- 前端即時顯示進度條（百分比 + ETA）、結果表格（CORRECT 綠 / WRONG 紅 / MISSING 琥珀）
- `CHECK_PRESETS` 定義各專案的 keywords、formatRules、columns
